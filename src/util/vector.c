...
void vec_reserve(Vec* vec, int new_max_size) {
    ...
    // Zero new memory
    memset(((char*) vec->elems) + vec->max_size * vec->elem_bytes, 0,
           vec->elem_bytes * (new_max_size - vec->max_size));
...
Vec* vec_insert(Vec* vec1, Vec* vec2, int insert_index) {
    ...
    memmove(vec1->elems + to_index * vec1->elem_bytes, from_address,
            (vec1->size - insert_index) * (vec1->elem_bytes));
    // Copy in vec2
